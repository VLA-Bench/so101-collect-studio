"""井字棋 300 条实例:冻结清单、双任务映射、同步、进度与 episode 身份。"""
import hashlib
import tempfile
import unittest
...
TASKS = [
    {"slug": "black_cube", "prompt": "black instruction", "set": "tic_tac_toe"},
    {"slug": "white_cube", "prompt": "white instruction", "set": "tic_tac_toe"},
]


class FrozenScheduleTest(unittest.TestCase):
    def test_manifest_has_300_unique_instances_and_two_task_mapping(self):
        self.assertEqual(
        ...
        self.assertEqual({row["task_slug"] for row in rows}, {"black_cube", "white_cube"})
        for row in rows:
    ...
    def test_progress_ignores_instance_saved_under_wrong_language_task(self):
        row = tic_tac_toe.load_instances()[0]
        wrong_slug = "white_cube" if row["task_slug"] == "black_cube" else "black_cube"
...
class TicTacToeApiTest(unittest.TestCase):
    ...
    def test_set_current_maps_instance_to_one_of_two_tasks(self):
        ...
        self.assertEqual(current["task_slug"], "black_cube")
        self.assertEqual(current["instruction"], "black instruction")
    ...
    def test_record_start_attaches_instance_metadata(self):
        ...
        with mock.patch.object(self.server.rec, "rec_start", side_effect=lambda task: captured.append(task)), \
            ...
            out = self.server.rec_start(self.server.RecStartReq(
                task_set="tic_tac_toe", task_slug="black_cube",
            ))
    ...
    def test_record_start_rejects_wrong_side_task(self):
        ...
        with self.assertRaises(HTTPException) as caught:
            self.server.rec_start(self.server.RecStartReq(
                task_set="tic_tac_toe", task_slug="white_cube",
            ))
    ...
    def test_record_start_rejects_completed_instance(self):
        ...
        self.server.tic_tac_toe_set_current(self.server.TicTacToeCurrentReq(selection_index=0))
        episode = {
            "status": "saved", "task_set": "tic_tac_toe", "task_slug": "black_cube",
            "tic_tac_toe": tic_tac_toe.episode_metadata(row),
        }
...
class RecorderAndExporterMetadataTest(unittest.TestCase):
    def test_recorder_meta_keeps_tic_tac_toe_identity(self):
        ...
        service.cur_task = {
            "slug": "black_cube", "set": "tic_tac_toe", "prompt": "black instruction",
        ...
        self.assertEqual(meta["task_slug"], "black_cube")
